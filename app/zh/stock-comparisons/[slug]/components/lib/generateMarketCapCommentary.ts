...
export function generateMarketCapCommentary({
  ...
  if (ratio > 1.5) {
    return `${stockOneSymbol}的市值（${stockOneMarketCapString}亿${stockOneCurrency}）显著高于${stockTwoSymbol}的（${stockTwoMarketCapString}亿${stockTwoCurrency}），显示出其在公司规模上的明显领先地位。`;
  }

  if (ratio < 1 / 1.5) {
    return `${stockTwoSymbol}的市值（${stockTwoMarketCapString}亿${stockTwoCurrency}）远超${stockOneSymbol}的（${stockOneMarketCapString}亿${stockOneCurrency}），表明${stockTwoSymbol}在体量上占据显著优势。`;
  }
