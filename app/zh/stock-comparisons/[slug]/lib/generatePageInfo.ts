...
export function generatePageInfo({
  ...
  const title = `${stockOneSymbol}与${stockTwoSymbol}：股票综合对比`;
  const pathname = `/zh/stock-comparisons/${slug}`;

  const description = `${stockOneSymbol}与${stockTwoSymbol}深度对比分析。从公司概况、历史表现、盈利能力、财务实力、成长性、股息和估值等多维度对比，辅助您的判断。`;
