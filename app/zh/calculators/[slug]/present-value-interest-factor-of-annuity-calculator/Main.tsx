...
export function Main({ pathname, className }: MainProps) {
  return (
    <main className={clsx(styles.main, className)}>
      ...
      <Article>
        <P>
          年金现值系数 (Present Value Interest Factor of an Annuity，简称 PVIFA)
          是金融领域中用于评估未来一系列等额现金流现值的重要工具，广泛应用于投资决策、贷款分析和退休规划。本文将深入讲解
          PVIFA
        ...
        <Section ariaLabelledby="what-is-present-value-interest-factor-of-annuity">
          ...
          <Ul>
            ...
            <Ul.Li>
              ...
              ：指未来一笔或一系列现金流在今天的价值。由于货币具有时间价值——今天的1元钱比未来的1元钱更有价值，因为它能用于投资并产生回报——未来的现金流需要折算回当前时点
              (即“折现”)，才能进行有意义的比较和决策。
            </Ul.Li>
          </Ul>
          <P>
            年金现值系数 (PVIFA)
            本身不是金额，而是一个系数或乘数。它用于便捷地计算一系列等额未来付款
            (即普通年金) 在今天的总价值。具体来说，PVIFA 是指在给定的折现率
            (利率) 和支付期数下，未来每期支付1单位货币 (例如1元)
            所对应的普通年金的现值总和。
          </P>
          <P>
            投资者和分析师可以利用 PVIFA
        ...
        <Section ariaLabelledby="how-to-calculate-pvifa">
          ...
          <P>
            值得注意的是，当利率 r 为零时，上述公式在数学上是无定义的
            (因分母为零)。在这种情况下，PVIFA
          ...
          <Ul>
            ...
            <Ul.Li>
              <Strong>期数 (n)</Strong>：期数越多，PVIFA 越大
              (在利率为正的情况下)。这是因为有更多笔未来付款被纳入现值计算。但随着期数增加，PVIFA
        ...
        <Section ariaLabelledby="using-pvifa-calculator">
          ...
          <P>
            ...
            计算器或电子表格软件 (如Excel中的PV函数)
            可以显著提高效率并减少计算错误。在电子计算器和电子表格软件普及之前，人们通常通过查阅预先印制的PVIFA表格来获取系数值。如今，虽然这些表格仍可见，但直接计算更为普遍和便捷。
          </P>
          <P>PVIFA 计算器通常要求输入：</P>
          <Ul>
            <Ul.Li>
        ...
        <Section ariaLabelledby="frequently-asked-questions">
          ...
          <P>
            ...
            设计的，即假定每期支付发生在期末。若处理的是期初年金 (Annuity Due)
            (即每期支付发生在期初)，其现值会略高，因为每笔款项都提前一个周期收到。期初年金的现值系数可以通过将期末年金的
            PVIFA 乘以 (1+r) 来计算：
          </P>
